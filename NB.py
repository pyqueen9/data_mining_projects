...
def calculateNominalAttModes():
    index = 0
    count = 0 
    # will use counts to find the mode of nominal atts
    for j in range(len(atts)):
        newarr = [0]*len(atts[j])
        nominalAttCounts.append(newarr)
    for i in range(len(instances)):  
        for k in range(len(instances[i])):  
            if atts[k][0] != "numeric": #  assume a nominal att if label is not a numeric
                if instances[i][k] != "?":
...
def calculateEntropyOfClassAtt(pos):
    # last attribute is the class - ignore mode although for binary class can get counts from mode..
...
def entropyDiscretize(classEnt, pos, attsValues, left):
    ...
    if maxGain > 0.05:
        #print("Starting on the left array")
        newArray = getAttsValues(4,1,bestSplit, attsValues) # new attValues
        if left == 1:
            rightArray = newArray[1]
        entropyDiscretize(classEnt, pos, newArray[0], 0)
    else: 
        #print("This is the max for left array")
        return ;
    if left == 1:
        if maxGain > 0.05:
        #print("Starting on right array")
            newArray = getAttsValues(4,1,bestSplit, rightArray) # new attValues
            entropyDiscretize(classEnt, pos, rightArray,1)
        else: 
            #print("This is the max for right array")
            return bestSplits
